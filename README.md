...
Free Claude Code routes Anthropic Messages API traffic from Claude Code to NVIDIA NIM, OpenRouter, DeepSeek, Alibaba Bailian, LM Studio, llama.cpp, or Ollama. It keeps Claude Code's client-side protocol stable while letting you choose free, paid, or local models.
...
- Six provider backends: NVIDIA NIM, OpenRouter, DeepSeek, Alibaba Bailian, LM Studio, llama.cpp, and Ollama.
...
| Alibaba Bailian | `bailian/...` | Anthropic Messages | `BAILIAN_API_KEY` | `https://dashscope.aliyuncs.com/api/v1` |
...
<details>
<summary><b>Alibaba Bailian (DashScope)</b></summary>

Get a key at [Aliyun Model Studio](https://help.aliyun.com/zh/model-studio/developer-reference/get-api-key).
...
**403 Error Auto-Fallback Feature**: When a model returns 403, the system automatically tries the next model in your configured list.

```dotenv
# Configure fallback models (comma-separated)
BAILIAN_MODELS="qwen-max,qwen-plus,qwen-turbo,qwen-long"
```

Popular examples:

- `bailian/qwen-max` - Best for complex reasoning
- `bailian/qwen-plus` - Balanced performance and cost
- `bailian/qwen-turbo` - Fast responses for simple tasks
- `bailian/qwen-long` - Extended context window support

When you configure multiple models, the proxy will automatically cycle through them if any return 403 errors, ensuring high availability.

See [docs/bailian_provider.md](docs/bailian_provider.md) for detailed documentation.
