...
def _create_bailian(config: ProviderConfig, settings: Settings) -> BaseProvider:
    from providers.bailian import BailianProvider
    
    # Parse the comma-separated list of fallback models
    bailian_models = []
    if settings.bailian_models:
        bailian_models = [m.strip() for m in settings.bailian_models.split(",") if m.strip()]
    
    return BailianProvider(config, bailian_models=bailian_models)
